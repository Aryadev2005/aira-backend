  ...
  try {
    ...
    if (!user) {
      ...
      try {
        ...
        // Email already exists - try to find by email and update firebase_uid
        if (
          createErr.code === "P2002" &&
          createErr.meta?.target?.includes("email")
        ) {
          ...
          if (user) {
            // Update the firebase_uid to match the current token
            ...
            logger.info(
              { userId: user.id },
              "Updated firebase_uid for existing user",
            );
          }
        } else {
          throw createErr;
