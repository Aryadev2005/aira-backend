...
export const createEntry = async (
  ...
  try {
    ...
    getTimingIntelligence({
      ...
      .then((timing) => cache.set(`timing:entry:${entryId}`, timing, 43200))
      .catch((err) =>
        logger.warn(
          { err, entryId },
          "Entry timing suggestion failed — non-fatal",
        ),
      );
  ...
  try {
    ...
    if (cached) {
      return success(reply, {
        timing: JSON.parse(cached as string),
        fromCache: true,
      });
    }
    ...
    await cache.set(cacheKey, timing, 43200).catch(() => {});
