...
export const WEEKLY_REPORT_CACHE_KEY = (userId: string) =>
...
export const generateWeeklyReport = async (
  ...
  const niche = Array.isArray(user.niches)
    ? (user.niches as string[])[0]
    : "general";
...
export const getOrGenerateWeeklyReport = async (
  userId: string,
): Promise<WeeklyReport & { fromCache: boolean }> => {
  const key = WEEKLY_REPORT_CACHE_KEY(userId);
  try {
    const cached = await cache.get(key);
    if (cached) {
      return {
        ...(JSON.parse(cached as string) as WeeklyReport),
        fromCache: true,
      };
    }
  } catch (_) {
  ...
  try {
    await cache.set(key, JSON.stringify(report), CACHE_TTL);
  } catch (_) {
    /* non-fatal */
  }
...
export const preGenerateForActiveUsers = async (): Promise<{
  generated: number;
  failed: number;
}> => {
  ...
  try {
    ...
    logger.warn(
      { err: err.message },
      "weeklyReport: failed to fetch active users",
    );
  ...
  for (const userId of activeUserIds) {
    try {
      ...
      await cache.set(key, JSON.stringify(report), CACHE_TTL).catch(() => {});
      generated++;
      ...
      logger.warn(
        ...
        "weeklyReport: pre-generation failed for user",
      );
    }
  }
